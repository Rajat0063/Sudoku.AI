...
import { parseImageToGrid } from "@/lib/tesseract-ocr";
...
const MAX_HINTS = 3;
// Number of mistakes required to unlock AI Solve.
const AI_SOLVE_UNLOCK_AT = 5;
...
export function SudokuGame() {
  ...
  const [scanningUpload, setScanningUpload] = useState(false);
  ...
  // auto-start camera when modal opens; show friendly error if access denied
  useEffect(() => {
    let mounted = true;
    const start = async () => {
      ...
      try {
        ...
        const msg =
          name === "NotAllowedError" || name === "PermissionDeniedError"
            ...
            : getMessage(err) || "Could not access camera.";
        toast.error(msg);
      }
    };
    start();
  ...
  const newGame = useCallback(
    (d: Difficulty = difficulty) => {
      ...
      // select first editable cell so keyboard input works immediately
      ...
      setSelected(sel);
      setScannedLoaded(false);
      // ensure the game container has focus so keyboard input works immediately
      setTimeout(() => {
        try {
  ...
  useEffect(() => {
    newGame("medium"); /* eslint-disable-next-line */
  }, []);
  ...
  // Win detection
  useEffect(() => {
    if (!solution.length || won || !running) return;
    if (boardsEqual(board, solution)) {
      setWon(true);
      setRunning(false);
      toast.success("Solved! 🎉", {
        description: `Time ${fmtTime(seconds)} · ${mistakes} mistakes`,
      });
    }
  }, [board, solution, won, seconds, mistakes, running]);

  const isFixed = (r: number, c: number) => puzzle[r][c] !== 0;

  const setCellValue = useCallback(
    (r: number, c: number, n: number) => {
      if (puzzle[r][c] !== 0 || won || solving) return;
      const key = `${r},${c}`;
      if (n === 0) {
        setBoard((b) => {
  ...
  const handleHint = async () => {
    ...
    setAiLoading(true);
    try {
      const res = await hintFn({ data: { puzzle, current: board } });
      ...
      const hint = resData.hint ?? "";
      setAiText(hint);
      setHintsLeft((h) => h - 1);
      // If server returned a suggested cell, fill it immediately
      const row = resData.row;
      const col = resData.col;
      ...
      toast.error(getMessage(e) || "Hint failed");
    } finally {
      setAiLoading(false);
    }
  };
  ...
  const handleAISolve = async () => {
    ...
    for (let r = 0; r < 9; r++)
      for (let c = 0; c < 9; c++)
        if (board[r][c] !== target[r][c]) cells.push([r, c]);
  ...
  const handleImageFile = async (file: File) => {
    ...
    // read file to data url and reuse scanDataUrl
    setScanningUpload(true);
    try {
      ...
      const grid = await scanDataUrl(dataUrl);
      setReviewGrid(grid);
    } catch (e: unknown) {
      toast.error(getMessage(e) || "Scan failed");
    } finally {
      setScanningUpload(false);
    }
  };

  // Scans a data URL with server OCR first, then Tesseract fallback. Returns grid or throws.
  const scanDataUrl = async (dataUrl: string) => {
    // Prefer server-side OCR (OCR.space)
    try {
      const { grid } = await scanFn({ data: { image: dataUrl } });
      const placed = grid.flat().filter((v) => v !== 0).length;
      if (placed >= 1) return grid;
    } catch (sErr) {
      console.debug("Server OCR failed, falling back to Tesseract", sErr);
    }
    // client-side Tesseract
    const tess = await parseImageToGrid(dataUrl);
    return tess.grid;
  };
  ...
  return (
    <div
      ref={containerRef}
      tabIndex={0}
      className="min-h-screen w-full px-4 py-8 md:py-12"
    >
          ...
          <div>
            <div className="flex items-center">
              <img
                src="/favicon.svg"
                alt="logo"
                className="mr-3 h-10 w-10 rounded-md animate-pulse"
                style={{
            ...
            <p className="mt-1 text-sm text-muted-foreground">
              Backtracking solver · AI hints · Image &amp; camera scan.
            </p>
          </div>
          ...
          <div className="flex flex-col gap-4">
            ...
            <div className="grid grid-cols-2 gap-3 sm:grid-cols-3 lg:grid-cols-6">
              <ActionBtn
                ...
                onClick={handleHint}
                disabled={aiLoading || hintsLeft <= 0 || won}
              >
                {aiLoading ? (
              ...
              <ActionBtn
                ...
                disabled={
                  solving ||
                  won ||
                  (!scannedLoaded && mistakes < AI_SOLVE_UNLOCK_AT)
                }
                variant="primary"
                title={
                  !scannedLoaded && mistakes < AI_SOLVE_UNLOCK_AT
                    ? `Unlocks after ${AI_SOLVE_UNLOCK_AT} mistakes (${mistakes}/${AI_SOLVE_UNLOCK_AT})`
                    : undefined
                }
              >
                {solving
                  ? "Solving…"
                  : !scannedLoaded && mistakes < AI_SOLVE_UNLOCK_AT
                    ? `AI Solve 🔒 ${mistakes}/${AI_SOLVE_UNLOCK_AT}`
                    : "AI Solve"}
              </ActionBtn>
              <ActionBtn
                icon={
                ...
                disabled={scanningUpload || scanningCamera || solving}
                variant="primary"
              >
                {scanningUpload ? "Scanning…" : "Upload"}
              </ActionBtn>
              <ActionBtn
                icon={
                  scanningCamera ? (
                ...
                onClick={async () => setCameraOpen(true)}
                disabled={scanningUpload || scanningCamera || solving}
                variant="primary"
              >
                {scanningCamera ? "Scanning…" : "Camera"}
              </ActionBtn>
              <ActionBtn
            ...
            {cameraOpen && (
              ...
              >
                <div
                  className="relative w-full max-w-xl rounded-2xl border border-border bg-card p-4"
                  onClick={(e) => e.stopPropagation()}
                >
                  <div className="flex items-center justify-between mb-3">
                    <h3 className="font-display text-lg font-bold">Camera</h3>
                    <button
                      onClick={() => setCameraOpen(false)}
                  ...
                  <div className="flex flex-col items-center gap-3">
                    ...
                    <div className="flex gap-2">
                      ...
                      <button
                        onClick={async () => {
                          ...
                          try {
                            ...
                            toast.error(getMessage(err) || "Scan failed");
                          } finally {
                      ...
                      <button
                        ...
                        className="rounded-md border px-3 py-2 text-destructive"
                      >
                        Close
                      </button>
                    </div>
            ...
            <p className="text-xs text-muted-foreground">
              Arrow keys to navigate · 1–9 to enter · Backspace to clear ·
              Upload or use Camera to scan a puzzle · AI Solve unlocks after{" "}
              {AI_SOLVE_UNLOCK_AT} mistakes (except when puzzle is loaded via
              Upload/Camera)
            </p>
          </div>
        </div>
        ...
        <ReviewScannedModal
          ...
          onConfirm={(g) => {
            ...
            toast.success("Puzzle loaded!", {
              description: "Hit AI Solve or play it yourself.",
            });
            return null;
          }}
        />
      ...
      <div className="grid grid-cols-9 gap-0">
        {board.map((row, r) =>
          row.map((val, c) => {
            ...
            const borderR =
              (c + 1) % 3 === 0 && c !== 8
                ? "border-r-2 border-r-grid-strong"
                : "border-r border-r-grid/40";
            const borderB =
              (r + 1) % 3 === 0 && r !== 8
                ? "border-b-2 border-b-grid-strong"
                : "border-b border-b-grid/40";
            const borderL = c === 0 ? "" : "";
            const borderT = r === 0 ? "" : "";
...
function Stat({
  ...
  return (
    <div>
      ...
      <div
        className={`mt-1 font-display text-2xl font-bold ${danger ? "text-destructive" : ""}`}
      >
        {value}
      </div>
    </div>
  ...
  const conflicts = useMemo(() => {
    ...
    for (let r = 0; r < 9; r++) {
      for (let c = 0; c < 9; c++) {
        ...
        // row
        for (let cc = 0; cc < 9; cc++)
          if (cc !== c && grid[r][cc] === v) set.add(`${r},${c}`);
        // col
        for (let rr = 0; rr < 9; rr++)
          if (rr !== r && grid[rr][c] === v) set.add(`${r},${c}`);
        // box
  ...
  useEffect(() => {
    const onKey = (e: KeyboardEvent) => {
      ...
      if (e.key === "ArrowUp") {
        ...
        setSel([Math.min(8, r + 1), c]);
        e.preventDefault();
      } else if (e.key === "ArrowLeft") {
        setSel([r, Math.max(0, c - 1)]);
        e.preventDefault();
      } else if (e.key === "ArrowRight") {
        setSel([r, Math.min(8, c + 1)]);
        e.preventDefault();
        ...
        <div className="mb-3 flex items-center justify-between">
          <div>
            <h2 className="font-display text-xl font-bold">
              Review scanned puzzle
            </h2>
            <p className="mt-0.5 text-xs text-muted-foreground">
              OCR isn't perfect. Tap any cell and type 1–9 to fix mistakes. Red
              = duplicate.
            </p>
          </div>
          <button
